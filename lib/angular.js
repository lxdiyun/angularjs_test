...
function $HttpBackendProvider() {
  this.$get = ['$browser', '$window', '$document', function($browser, $window, $document) {
    return createHttpBackend($browser, createXhr, $browser.defer, $window.angular.callbacks, $document[0], $window);
  }];
}

function createHttpBackend($browser, createXhr, $browserDefer, callbacks, rawDocument, $window) {
  var ABORTED = -1;
  ...
  return function(method, url, post, callback, headers, timeout, withCredentials, responseType) {
    ...
    if (lowercase(method) == 'jsonp') {
      ...
      $window['angular_callbacks' + callbackId] = function (data) {
        $window.angular.callbacks[callbackId](data);
        delete $window['angular_callbacks' + callbackId];
      };
